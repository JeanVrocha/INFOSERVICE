// SplashCursor - Vanilla JS version adapted from provided React component
// Creates a full-screen WebGL fluid canvas reacting to pointer/touch
// Usage: initSplashCursor({ /* optional overrides */ })
...
(function(){
  function initSplashCursor(options){
    const config = Object.assign({
      ...
      BACK_COLOR: { r: 0.02, g: 0.0, b: 0.06 },
      TRANSPARENT: true
    }, options || {});
    ...
    const divergenceShader = compileShader(gl.FRAGMENT_SHADER, `
      precision mediump float; precision mediump sampler2D; varying vec2 vUv, vL, vR, vT, vB; uniform sampler2D uVelocity;
      void main () {
        float L = texture2D(uVelocity, vL).x; float R = texture2D(uVelocity, vR).x; float T = texture2D(uVelocity, vT).y; float B = texture2D(uVelocity, vB).y; vec2 C = texture2D(uVelocity, vUv).xy;
    ...
    const vorticityShader = compileShader(gl.FRAGMENT_SHADER, `
      ...
      void main () {
        ...
        force /= length(force) + 0.0001; force *= curl * C; force.y *= -1.0; vec2 velocity = texture2D(uVelocity, vUv).xy; velocity += force * dt; velocity = min(max(velocity, -1000.0), 1000.0); gl_FragColor = vec4(velocity, 0.0, 1.0);
      }
    `);

    const pressureShader = compileShader(gl.FRAGMENT_SHADER, `
      precision mediump float; precision mediump sampler2D; varying vec2 vUv, vL, vR, vT, vB; uniform sampler2D uPressure, uDivergence;
    ...
    const gradientSubtractShader = compileShader(gl.FRAGMENT_SHADER, `
      precision mediump float; precision mediump sampler2D; varying vec2 vUv, vL, vR, vT, vB; uniform sampler2D uPressure, uVelocity;
    ...
    function createDoubleFBO(w, h, internalFormat, format, type, param) {
      ...
      return { width: w, height: h, texelSizeX: fbo1.texelSizeX, texelSizeY: fbo1.texelSizeY,
        get read(){ return fbo1; }, set read(v){ fbo1 = v; }, get write(){ return fbo2; }, set write(v){ fbo2 = v; }, swap(){ let t=fbo1; fbo1=fbo2; fbo2=t; } };
    }
    ...
    function updateKeywords(){ let displayKeywords = []; if (config.SHADING) displayKeywords.push('SHADING'); displayMaterial.setKeywords(displayKeywords); }
    ...
    function HSVtoRGB(h,s,v){ let r,g,b,i,f,p,q,t; i=Math.floor(h*6); f=h*6-i; p=v*(1-s); q=v*(1-f*s); t=v*(1-(1-f)*s); switch(i%6){case 0:r=v;g=t;b=p;break;case 1:r=q;g=v;b=p;break;case 2:r=p;g=v;b=t;break;case 3:r=p;g=q;b=v;break;case 4:r=t;g=p;b=v;break;case 5:r=v;g=p;b=q;break;default:break;} return {r,g,b}; }
    function wrap(value,min,max){ const range = max - min; if (range === 0) return min; return ((value - min) % range) + min; }
    function getResolution(resolution){ let aspectRatio = gl.drawingBufferWidth / gl.drawingBufferHeight; if (aspectRatio < 1) aspectRatio = 1.0 / aspectRatio; const min = Math.round(resolution); const max = Math.round(resolution * aspectRatio); if (gl.drawingBufferWidth > gl.drawingBufferHeight) return { width: max, height: min }; else return { width: min, height: max }; }
    ...
    // Interactions
    window.addEventListener('mousedown', e => { const p = pointers[0]; const x = scaleByPixelRatio(e.clientX), y = scaleByPixelRatio(e.clientY); updatePointerDownData(p, -1, x, y); clickSplat(p); });
    document.body.addEventListener('mousemove', function firstMM(e){ const p=pointers[0]; const x=scaleByPixelRatio(e.clientX), y=scaleByPixelRatio(e.clientY); const color=generateColor(); updateFrame(); updatePointerMoveData(p, x, y, color); document.body.removeEventListener('mousemove', firstMM); });
    window.addEventListener('mousemove', e => { const p=pointers[0]; const x=scaleByPixelRatio(e.clientX), y=scaleByPixelRatio(e.clientY); updatePointerMoveData(p, x, y, p.color); });
    document.body.addEventListener('touchstart', function firstTS(e){ const touches=e.targetTouches; const p=pointers[0]; for (let i=0;i<touches.length;i++){ const x=scaleByPixelRatio(touches[i].clientX), y=scaleByPixelRatio(touches[i].clientY); updateFrame(); updatePointerDownData(p, touches[i].identifier, x, y); } document.body.removeEventListener('touchstart', firstTS); });
    window.addEventListener('touchstart', e => { const touches=e.targetTouches; const p=pointers[0]; for (let i=0;i<touches.length;i++){ const x=scaleByPixelRatio(touches[i].clientX), y=scaleByPixelRatio(touches[i].clientY); updatePointerDownData(p, touches[i].identifier, x, y); } });
    window.addEventListener('touchmove', e => { const touches=e.targetTouches; const p=pointers[0]; for (let i=0;i<touches.length;i++){ const x=scaleByPixelRatio(touches[i].clientX), y=scaleByPixelRatio(touches[i].clientY); updatePointerMoveData(p, x, y, p.color); } }, { passive: true });
    ...
    return { destroy(){ try{ document.body.removeChild(container); }catch(_){} } };
  }
